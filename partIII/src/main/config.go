...
func (cfg *config) checkCommittedIndex(index int, command interface{}, expectedServers int) {
	cmd, cmdInitialized := command.(Command)
...
func (cfg *config) replicateOne(server int, cmd Command, expectedServers int) (
	index int) {
	...
	for time.Since(t0).Seconds() < 10 {
		...
		if committed {
			...
			for i := 0; i < len(cfg.pbservers); i++ {
				...
				if ok {
					if cmd2, ok2 := cmd1.(Command); ok2 && reflect.DeepEqual(cmd2, cmd) {
						nReplicated++
					}
				}
			}
